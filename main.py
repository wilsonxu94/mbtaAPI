...
def populateMaps():
	...
	global routeStopMap #map of routes with the stops on it
	...
	del routeToRouteMapFinal[brokenRoute]
	#IF the key is the broken route, remove the key from the map
	for key in routeToRouteMapFinal:

			tempList = routeToRouteMapFinal[key] 
			if(brokenRoute in tempList):
				tempList.remove(brokenRoute)

			routeToRouteMapFinal[key] = tempList



	#IF the value of the key contains the broken route, remove that value from the list of transfer
